...
const CreateInvoice = () => {
  ...
  // State for PO lookup when no purchaseOrderId is provided
  ...
  const handleSearchPO = async () => {
    ...
    try {
      ...
      if (po.poStatus !== 'CONFIRMED' && po.orderStatus !== 'CONFIRMED') {
        setPoSearchError('This purchase order is not confirmed yet. Only confirmed orders can be invoiced.');
        return;
      }
      ...
      setPoSearchError('Failed to find purchase order. Please check the order number and try again.');
    } finally {
  ...
  useEffect(() => {
    const fetchPurchaseOrder = async () => {
      // Don't fetch if purchaseOrderId is missing or invalid
      const poIdToFetch = purchaseOrderId || selectedPOId;
      if (!poIdToFetch || poIdToFetch === 'undefined') {
        return;
      }
      ...
      try {
        ...
        setPaymentTermId(data?.paymentTerms?.paymentTermId || '');
      } catch (error) {
  ...
  }, [supplierId, purchaseOrderId, selectedPOId]);
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const response = await FileUploadService.uploadSupplierFile(supplierId, file);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      // Get the PO ID to use
      const poIdToUse = purchaseOrderId || selectedPOId;

      const invoicePayload = {
        title: values.title,
        invoiceNo: values.invoiceNo,
        purchaseOrderId: Number(poIdToUse),
        ...
        <Col md="12">
          <div className="card h-100 shadow-sm" style={{ borderRadius: '12px', border: 'none' }}>
            <div className="card-body" style={{ padding: '24px' }}>
              ...
              {/* Order Number Input when no purchaseOrderId is provided */}
                            ...
                            >
                              <Row>
                                <Col md="6">
                                  ...
                                  <p>
                                    <strong>Company:</strong> {poData.company?.displayName || 'N/A'}
                                  </p>
                                </Col>
                                <Col md="6">
                                  <p>
                                    ...
                                    <tr key={item.purchaseOrderDetailId || index}>
                                      ...
                                      >
                                        {item.partDescription
                                          ? item.partDescription.split(' ').slice(0, 3).join(' ') +
                                      ...
                                      <td
                                        style={{
                                          ...
                                          fontSize: '0.75rem',
                                          fontWeight: '600',
                                          color: item.availableQuantity > 0 ? '#28a745' : '#dc3545',
                                        }}
                                      >
