...
const LocationManagement = () => {
  ...
  const fetchLocations = async () => {
    try {
      ...
      if (debouncedSearchTerm.trim() === '') {
        response = await LocationService.getAllLocation(companyId, {}, pageDto);
      } else {
        response = await LocationService.getAllLocation(
          companyId,
  ...
  const handleEdit = (locationId) => {
    navigate(`/location-registration/${locationId}`);
  };
  ...
  const handleDeleteLocation = async (locationId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await LocationService.handleDeleteLocation(companyId, locationId);
  ...
  const renderActionButtons = (cell, row) => (
    <div className="d-flex justify-content-center">
      <button
        ...
        onClick={() => handleEdit(row.locationId)}
      >
        <Edit size={14} />
      </button>
      <button
        id={`delete-tooltip-${row.locationId}`}
        type="button"
        className="btn btn-sm btn-danger action-button-delete"
        onClick={() => handleDeleteLocation(row.locationId)}
      >
        <Trash size={14} />
      </button>
    </div>
        ...
        <Col md="12">
          <ComponentCard
            title={
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  <h4 className="mb-1">Location Management</h4>
          ...
          >
            ...
            >
              <div className="d-flex align-items-center gap-3">
                ...
                <div className="stats-info d-flex align-items-center gap-2">
                  ...
                  <span style={{ fontSize: '14px', fontWeight: '600', color: '#495057' }}>
                    Total Locations: <span style={{ color: '#009efb' }}>{locations.length}</span>
                  </span>
                </div>
              </div>
              <button
              ...
              >
                <i className="fas fa-plus me-2"></i>Add New Location
              </button>
            </div>
            <div className="table-responsive">
              <BootstrapTable
                ...
                hover
                condensed
                pagination={locations.length > pageSize}
                options={options}
